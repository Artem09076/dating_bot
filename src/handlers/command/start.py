...
async def start(message: Message, state: FSMContext) -> None:
    main_menu =[
        ...
        [InlineKeyboardButton(text="Редактировать анкету", callback_data='change_form')],
        [InlineKeyboardButton(text="Рейтинг", callback_data='rating')], 
        [InlineKeyboardButton(text="Помощь", callback_data='help')]
    ]
    reply_markup = InlineKeyboardMarkup(inline_keyboard=main_menu)
    await message.answer(render('start.jinja2', user=message.from_user), reply_markup=reply_markup)
