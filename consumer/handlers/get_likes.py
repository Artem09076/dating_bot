...
from src.model.model import Like, User
...
async def process_check_likes(body: dict):
    ...
    async with async_session() as db:
        stmt = select(Like.from_user_id).where(
            Like.to_user_id == user_id, Like.is_mutual.is_(None)
        )
        ...
        if not from_user_ids:
            response = {"user_id": user_id, "likes": []}
        else:
            ...
            response = {"user_id": user_id, "likes": users_data}
